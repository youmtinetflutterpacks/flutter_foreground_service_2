...
class ForegroundServiceHandler {
  static const MethodChannel _mainChannel = const MethodChannel("dk.cachet.flutter_foreground_service/main", JSONMethodCodec());

  static MethodChannel? _fromBackgroundIsolateChannel;
  static Future<bool> get isBackgroundIsolate async => (await isBackgroundIsolateSetupComplete()) && (_fromBackgroundIsolateChannel != null);
  ...
  static Future<T> _invokeMainChannel<T>(String method, [dynamic arguments]) async {
    if (_fromBackgroundIsolateChannel == null) {
      ...
      return await _fromBackgroundIsolateChannel!.invokeMethod("fromBackgroundIsolate", {"method": method, "arguments": arguments});
    }
  }

  static final ForegroundServiceNotification notification = new ForegroundServiceNotification._(_invokeMainChannel);
  ...
  static Future<void> setupIsolateCommunication(Function(dynamic message) messageHandler) async {
    ...
    if (_receivePort == null) {
      ...
      _receivePort!.listen((data) {
        ...
        () async {
          if (callHandler == null) {
            debugPrint("${DateTime.now()}: ${await isBackgroundIsolate ? "Background" : "Main"} isolate "
                "received message $data, but receiveHandler is null.");
          }
        }();
      });

      final String portMappingName = (await isBackgroundIsolate) ? _BACKGROUND_ISOLATE_PORT_NAME : _MAIN_ISOLATE_PORT_NAME;
  ...
  static bool get isIsolateCommunicationSetup => ((_receivePort != null) && (_receiveHandler != null));

  static ReceivePort? _receivePort;

  static const String _MAIN_ISOLATE_PORT_NAME = "dk.cachet.flutter_foreground_service/MAIN_ISOLATE_PORT";
  static const String _BACKGROUND_ISOLATE_PORT_NAME = "dk.cachet.flutter_foreground_service/BACKGROUND_ISOLATE_PORT";
  ...
  static Future<void> sendToPort(dynamic message) async {
    final SendPort? targetPort = IsolateNameServer.lookupPortByName((await isBackgroundIsolate ? _MAIN_ISOLATE_PORT_NAME : _BACKGROUND_ISOLATE_PORT_NAME));
  ...
  static Future<void> startForegroundService([Function? serviceFunction, bool holdWakeLock = false]) async {
    ...
    if (!(await isBackgroundIsolate)) {
      final setupHandle = PluginUtilities.getCallbackHandle(_setupForegroundServiceCallbackChannel)?.toRawHandle();

      await _invokeMainChannel("startForegroundService", <dynamic>[setupHandle, holdWakeLock]);
...
//      }
  ...
  static Future<Function?> getServiceFunction() async => PluginUtilities.getCallbackFromHandle(await _invokeMainChannel("getServiceFunctionHandle"));
  ...
  static Future<int> getServiceIntervalSeconds() async => await _invokeMainChannel("getServiceFunctionInterval");
  ...
  static Future<bool> getContinueRunningAfterAppKilled() async => await _invokeMainChannel("getContinueRunningAfterAppKilled");
  ...
  static Future<bool> isBackgroundIsolateSetupComplete() async => await _invokeMainChannel("isBackgroundIsolateSetupComplete");

  // see setServiceFunctionAsync
  static Future<bool> getServiceFunctionAsync() async => await _invokeMainChannel("getServiceFunctionAsync");
...
class ForegroundServiceNotification {
  ...
  Future<AndroidNotificationPriority> getPriority() async => _priorityFromString((await _invokeMainChannel("getNotificationPriority")) as String);
  ...
  Future<void> setPriority(AndroidNotificationPriority newPriority) async {
    await _invokeMainChannel("setNotificationPriority", <dynamic>[newPriority.name]);
  }
...
void _setupForegroundServiceCallbackChannel() async {
  const MethodChannel _callbackChannel = MethodChannel("dk.cachet.flutter_foreground_service/callback", JSONMethodCodec());

  ForegroundServiceHandler._fromBackgroundIsolateChannel = MethodChannel("dk.cachet.flutter_foreground_service/fromBackgroundIsolate", JSONMethodCodec());
  ...
  _callbackChannel.setMethodCallHandler((MethodCall call) async {
    ...
    await ForegroundServiceHandler._invokeMainChannel("backgroundIsolateCallbackComplete");
  });
...
class WrongIsolateException implements Exception {
  ...
  @override
  String toString() => "Throwing function was executed in the ${contextIsBackgroundIsolate ? "background" : "main"} isolate.";
}
